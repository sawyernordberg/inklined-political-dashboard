...
export default function EconomicPolicyPage() {
    ...
    async function loadData() {
      try {
        ...
        console.log('Economic data loaded successfully:', {
          ...
          integrated: !!fetchedData?.integrated,
        ...
        if (fetchedData?.stock) {
          console.log('Stock data structure:', {
            hasPresidentialData: !!fetchedData.stock.presidential_data,
            presidents: Object.keys(fetchedData.stock.presidential_data || {}),
            trumpData: fetchedData.stock.presidential_data?.["Donald Trump (2nd Term)"],
            trumpDailyData: fetchedData.stock.presidential_data?.["Donald Trump (2nd Term)"]?.daily_data?.length || 0
          });
        }
        
        console.log('Full data object:', fetchedData);
        setData(fetchedData);
        setLoading(false);
  ...
  console.log('Render - loading:', loading, 'error:', error, 'hasData:', !!data);
  ...
  if (loading) {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', fontFamily: 'system-ui' }}>
        ...
        <p>Loading Economic Dashboard...</p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', fontFamily: 'system-ui', textAlign: 'center', color: '#dc2626' }}>
        ...
        {data && (
          <div style={{ marginTop: '20px', textAlign: 'left', background: '#f5f5f5', padding: '20px', borderRadius: '8px' }}>
            <h3>Available Data Debug:</h3>
            <pre style={{ fontSize: '12px' }}>{JSON.stringify(data, null, 2).substring(0, 1000)}...</pre>
          </div>
        )}
      </div>
    );
