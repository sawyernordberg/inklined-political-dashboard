...
export interface CongressionalComposition {
  house: {
    republican: number;
    democratic: number;
    independent: number;
...
export interface HistoricalProductivityComparison {
  [key: string]: {
...
interface CongressPageClientProps {
  data: CongressData;
}

export default function CongressPageClient({ data }: CongressPageClientProps) {
                  ...
                  <div className="ranking-container">
                    ...
                    <div className="ranking-list">
                      {Object.entries(data.historical_productivity_comparison)
                        ...
                        .sort(([,a], [,b]) => {
                          const aData = a as { bills_passed_in_timeframe: number };
                          const bData = b as { bills_passed_in_timeframe: number };
                          return bData.bills_passed_in_timeframe - aData.bills_passed_in_timeframe;
                        })
                        .map(([congress, congressData], index) => {
                          const data = congressData as { bills_passed_in_timeframe: number };
                          return (
                            <div key={congress} className={`ranking-item ${congress === '119' ? 'current' : ''}`}>
                              ...
                              <div className="ranking-bills">{data.bills_passed_in_timeframe} bills</div>
                            </div>
                          );
                        })}
                    </div>
